...
public abstract class AbstractEEGSignalSource : MonoBehaviour
{
    ...
    public static AbstractEEGSignalSource GetInstance()
    {
        if (AbstractEEGSignalSource.instance == null)
        {
            // Assume only 1 implementation of EEG source exists
            AbstractEEGSignalSource.instance = 
                GameObject.FindObjectsOfType<MonoBehaviour>(true)
                .OfType<AbstractEEGSignalSource>()
                .FirstOrDefault();
        }
    ...
    public bool EndSession()
    {
        try
        {
            ...
            {
                throw new Exception("Overriden function CustomEndSession() failed");
            }
        }
        catch (Exception e)
        {
    ...
    // Returns true if source has started or is streaming; false - otherwise
...
public class CrossPlatformEEGSourceFactory
{
    public static AbstractEEGSignalSource GetInstance()
    {
        if (Application.platform == RuntimePlatform.Android)
        {
            return AndroidMindroveEEGSignalSource.GetInstance();
        }
        else
        {
            return DesktopMindroveEEGSignalSource.GetInstance();
        }
    }
...
// OLD DESKTOP EEG SOURCE CODE - works
